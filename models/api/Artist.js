...
Artist.prototype.getArtist = async function (ACESS_TOKEN, ARTIST_ID) {
  const config_art = {
    method: "get",
  ...
  return axios.request(config_art);
};
...
Artist.prototype.tokenRequest = async function (param) {
  ...
  const config = {
    ...
    headers: {
      Authorization:
        "Basic " +
        new Buffer.from(x.client_id + ":" + x.client_secret).toString("base64"),
    },
